...
>(({ className, size = "md", speed = "normal", ...props }, ref) => {
  return (
    <span className={cn("inline-block text-primary", speedClasses[speed], sizeClasses[size], className)}>
      <SpinnerPrimitive ref={ref} aria-hidden className="h-full w-full" {...props} />
    </span>
  );
});
