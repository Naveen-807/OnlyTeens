...
function contractAddressFromEnvOrDeploy(envName, deployName) {
...
async function assertLitActionCidResolvable(cid) {
...
function assertEvaluatorKeyFundedHint() {
  ...
  console.log(
    "  Zama evaluator private key configured (funding must be verified onchain)",
  );
}
...
async function main() {
  ...
  const passport = contractAddressFromEnvOrDeploy(
  ...
  const actionCid = process.env.NEXT_PUBLIC_SAFE_EXECUTOR_CID || process.env.SAFE_EXECUTOR_CID;
