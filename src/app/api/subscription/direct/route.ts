...
import { executeDirectSubscriptionFlow } from "@/lib/orchestration/directFlow";
...
export async function POST(req: Request) {
  try {
    ...
    if (!body?.session || !body?.familyId || !body?.teenAddress || !body?.serviceName) {
      return fail("BAD_REQUEST", "session, familyId, teenAddress, and serviceName are required", 400);
    }

    const cached = getCachedIdempotentResult(idempotencyKey);
    if (cached) return ok(cached as Record<string, unknown>);

    const result = await executeDirectSubscriptionFlow({
    ...
    return fail(mapErrorToCode(error), error?.message || "Direct subscription action failed", 500);
  }
}
